...
```{julia}
@model function model_exr06(N, x, y, σ_y)
	b ~ Normal(0, 5)
	m ~ Normal(0, 5)
...
chains_exr06 = sample(model_exr06(full_N, full_x, full_y, full_σ_y), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
```

diagnostics of chains:
```{julia}
gelmandiag(chains_exr06)
```
...
```{julia}
histogram2d(chains_exr06[:b], chains_exr06[:m], bins=N_bins, normalize=:pdf, color=:plasma, xlabel="b", ylabel="m")
```
...
# plot(chains_exr06, seriestype=:mixeddensity) # cannot change layout

p = plot(layout=(3, 2), legend=false)

density!(p, chains_exr06[:b], subplot=1, title="b")
density!(p, chains_exr06[:m], subplot=3, title="m")
histogram2d!(p, chains_exr06[:b], chains_exr06[:m], subplot=5, bins=N_bins, normalize=:pdf, color=:plasma, xlabel="b", ylabel="m")

density!(p, chains_exr06[:Y_bad], subplot=2, label=false, title="Y_bad")
density!(p, chains_exr06[:P_bad], subplot=4, label=false, title="P_bad")
density!(p, chains_exr06[:V_bad], subplot=6, label=false, title="V_bad")
```

```{julia}
bₑₓ₀₆ = mean(chains_exr06[:b]);
mₑₓ₀₆ = mean(chains_exr06[:m]);
plot(full_x, full_y, yerror=full_σ_y, seriestype=:scatter, label="")
plot!(x -> mₑₓ₀₂*x + bₑₓ₀₂, label="exr 02")
...
```{julia}
density(chains_exr06[:P_bad], label=false, title="P_bad with original error")
```
peak at 0.2, arguably 3-4 outliers in the data, so it makes sense

Now with ½ the error
```{julia}
chains_exr07 = sample(model_exr06(full_N, full_x, full_y, full_σ_y ./ 2), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
density(chains_exr07[:P_bad], label=false, title="P_bad with half error")
```
...
Threads.@threads for step ∈ 1:N_samples
	for chain ∈ 1:N_chains
		b = chains_exr06[:b][step, chain]
		m = chains_exr06[:m][step, chain]
		V_bad = chains_exr06[:V_bad][step, chain]
		P_bad = chains_exr06[:P_bad][step, chain]
		Y_bad = chains_exr06[:Y_bad][step, chain]
		for i ∈ 1:full_N
			ŷ_i = b + m * full_x[i]
...
```{julia}
chains_hogg_subset          = sample(model_exr06(subset_N, subset_x, subset_y, subset_σ_y),      NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
chains_hogg_subset_half_err = sample(model_exr06(subset_N, subset_x, subset_y, subset_σ_y ./ 2), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);

p = plot(layout=2)
histogram2d!(p, chains_hogg_subset[:b],          chains_hogg_subset[:m],          subplot=1, bins=N_bins, normalize=:pdf, color=:plasma, xlabel="b", ylabel="m", title="with original error")
histogram2d!(p, chains_hogg_subset_half_err[:b], chains_hogg_subset_half_err[:m], subplot=2, bins=N_bins, normalize=:pdf, color=:plasma, xlabel="b", ylabel="m", title="with half error")
```
...
$$
```{julia}
@model function model_exr12(N, x, y)
	b ~ Normal(0, 5)
	m ~ Normal(0, 5)
...
chains_same_σ = sample(model_exr12(subset_N, subset_x, subset_y), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
...
@model function model_exr13(N, x, y, σ_x, σ_y, ρ_xy)
...
chains_exr13 = sample(model_exr13(subset_N, subset_x, subset_y, subset_σ_x, subset_σ_y, subset_ρ_xy), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
```

```{julia}
bₑₓ₁₃ = mean(chains_exr13[:b]);
mₑₓ₁₃ = tan(mean(chains_exr13[:θ]));
