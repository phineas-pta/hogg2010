...
@model function hogg_model(N, x, y, σ_y)
	...
	for i ∈ 1:N
		...
		=#
		y[i] ~ MixtureModel(Normal, [(b + m*x[i], σ_y[i]), (Y_bad, σ_bad)], [1 - P_bad, P_bad]) # convenient constructors for mixture models
...
```{julia}
histogram2d(chains_hogg_full[:b], chains_hogg_full[:m], bins=N_samples÷10, color=:plasma, xlabel="b", ylabel="m")
```
...
histogram2d!(p, chains_hogg_full[:b], chains_hogg_full[:m], subplot=5, bins=N_samples÷10, color=:plasma, xlabel="b", ylabel="m")
...
p = plot(layout=2)
histogram2d!(p, chains_hogg_subset[:b],          chains_hogg_subset[:m],          subplot=1, bins=N_samples÷10, color=:plasma, xlabel="b", ylabel="m", title="with original error")
histogram2d!(p, chains_hogg_subset_half_err[:b], chains_hogg_subset_half_err[:m], subplot=2, bins=N_samples÷10, color=:plasma, xlabel="b", ylabel="m", title="with half error")
```
...
mean variances $\left(\sigma_{y_i}^2\right)_{i=1}^N$ of the true data set
```{julia}
mean(subset_σ_y .^ 2)
```
...
$$
```{julia}
@model function same_σ_model(N, x, y)
	b ~ Normal(0, 5)
	m ~ Normal(0, 5)
...
chains_same_σ = sample(same_σ_model(subset_N, subset_x, subset_y), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
