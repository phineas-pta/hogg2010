...
function print_information_criterion(data_model, mcmc_chains)
	...
	post_logℒ = [loglikelihood(data_model, row) for row ∈ eachrow(DataFrame(mcmc_chains))]
	...
	lpd = dropdims(logsumexp(post_logℒ .- log(size(post_logℒ)[1]); dims=1); dims=1)
	pD = dropdims(var(post_logℒ; dims=1, corrected=false); dims=1)
	WAIC = sum((-2) .* (lpd - pD))
	LPD = sum(lpd)
	penalty = sum(pD)
...
```{julia}
Sₑₓ₁₁[findlast(𝕩 -> 𝕩 > subsetNm2, χ²ₑₓ₁₁)]
```
and
```{julia}
Sₑₓ₁₁[findfirst(𝕩 -> 𝕩 < subsetNm2, χ²ₑₓ₁₁)]
```
...
@model function modelₑₓ₁₄(ℕ, ℤ, 𝕊)
	...
	Z_bad = [X_bad, Y_bad]
	V_bad = [
		V_bad_x                         ρ_bad * sqrt(V_bad_x*V_bad_y);
		ρ_bad * sqrt(V_bad_x*V_bad_y)   V_bad_y
	]
	...
	for i ∈ 1:ℕ
		...
		tmp1 = ℤ[i] - Z_bad
		tmp2 = 𝕊[i] + V_bad
		inlier = log1p(-P_bad) - .5*log(abs(Σᵢ²)) + log(abs(v⃗[1])) - .5 * Δᵢ^2 / Σᵢ²
		outlier = log(P_bad) - .5*log(det(tmp2)) - .5 * tmp1' * tmp2 * tmp1
		Turing.@addlogprob! logaddexp(inlier, outlier)
	end
end
...
Q = sum([z*z' for z ∈ Z´_centered]);
_, eigvecs = eigen(Q);
eigvecs_max = eigvecs[:, end];
mₑₓ₁₆ = eigvecs_max[end] / eigvecs_max[begin];
bₑₓ₁₆ = -mₑₓ₁₆ * Z̄´[1] + Z̄´[2];
plot(x´, y´, seriestype=:scatter, label=nothing)
plot!(𝕩 -> mₑₓ₁₆*𝕩 + bₑₓ₁₆, label=nothing)
```
slope = `{julia} mₑₓ₁₆` and intercept = `{julia} bₑₓ₁₆`
