  ...
  html:
    ...
    include-in-header:
      text: |
        ...
        .sourceCode {font-family: "Victor Mono", monospace;}
        </style>
...
```{julia}
const full_Z = [[full_x[i], full_y[i]] for i ∈ 1:full_N];
const full_S = [ # covariance tensor
	[
		full_σ_x[i]^2                          full_ρ_xy[i]*full_σ_x[i]*full_σ_y[i];
		full_ρ_xy[i]*full_σ_x[i]*full_σ_y[i]   full_σ_y[i]^2
	]
	for i ∈ 1:full_N
];
```
...
const subset_Z    = full_Z[   5:end];
const subset_S    = full_S[   5:end];
```
...
function plot_ellipses(N, x, y, Z, S)
	p = plot(x, y, seriestype=:scatter, label=nothing)
	for i ∈ 1:N
		covellipse!(p, Z[i], S[i], label=nothing)
	end
	return p
end
```
...
quick look at data
```{julia}
plot(full_x, full_y, xerror=full_σ_x, yerror=full_σ_y, seriestype=:scatter, label=nothing)
```

```{julia}
plot_ellipses(full_N, full_x, full_y, full_Z, full_S)
```
...
```{julia}
plot(subset_x, subset_y, yerror=subset_σ_y, seriestype=:scatter, label=nothing)
plot!(x -> mₑₓ₀₁*x + bₑₓ₀₁, label=nothing)
```
...
plot(full_x, full_y, yerror=full_σ_y, seriestype=:scatter, label=nothing)
...
@model function modelₑₓ₀₆(N, x, y, σ_y)
	...
	for i ∈ 1:N
		...
		#=
		 inlier = log1p(-P_bad) + logpdf(Normal(ŷ_i  , σ_y[i]), y[i])
...
```{julia}
summarystats(chainsₑₓ₀₆)[:, [:mean, :std, :mcse, :rhat]]
```
...
Threads.@threads for step ∈ 1:N_samples
	for chain ∈ 1:N_chains
		...
		for i ∈ 1:full_N
			ŷ_i = b + m * full_x[i]
			inliers_log_prob = log1p(-P_bad) + logpdf(Normal(ŷ_i, full_σ_y[i]), full_y[i])
...
summarystats(chainsₑₓ₀₉_½err)[:, [:mean, :std, :mcse, :rhat]]
```
...
plot(Sₑₓ₁₁, χ²ₑₓ₁₁, seriestype=:scatter, yaxis=:log, xlabel="S", ylabel="χ²", label=nothing)
hline!([subsetNm2], label="χ²=N-2")
vline!([mean(subset_σ_y .^ 2)], label="mean σ²")
```
...
plot(Sₑₓ₁₁bis, χ²ₑₓ₁₁bis, seriestype=:scatter, xlabel="S", ylabel="χ²", label=nothing)
hline!([subsetNm2], label="χ²=N-2")
```
...
chainsₑₓ₁₂ = sample(modelₑₓ₁₂(subset_N, subset_x, subset_y), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
summarystats(chainsₑₓ₁₂)[:, [:mean, :std, :mcse, :rhat]]
```
...
mₑₓ₁₂ = mean(chainsₑₓ₁₂[:m]);
Sₑₓ₁₂ = mean(chainsₑₓ₁₂[:S]);
plot(subset_x, subset_y, yerror=fill(sqrt(Sₑₓ₁₂), subset_N), seriestype=:scatter, label=nothing)
...
@model function modelₑₓ₁₃(N, Z, S)
	...
	for i ∈ 1:N
		Δ_i = dot(Z[i], v) - b*v[1] # orthogonal displacement of each data point from the line
		Σ²_i = dot(v', S[i], v) # orthogonal variance of projection of each data point to the line
		Turing.@addlogprob! -.5*(Δ_i^2 / Σ²_i + log(abs(Σ²_i)) + log1p(tan(θ)^2))
	end
end

chainsₑₓ₁₃ = sample(modelₑₓ₁₃(subset_N, subset_Z, subset_S), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
summarystats(chainsₑₓ₁₃)[:, [:mean, :std, :mcse, :rhat]]
```
...
plot_ellipses(subset_N, subset_x, subset_y, subset_Z, subset_S)
...
@model function modelₑₓ₁₇(N, Z, S)
	...
	for i ∈ 1:N
		Δ_i = dot(Z[i], v) - b*v[1] # orthogonal displacement of each data point from the line
		Σ²_i = dot(v', S[i], v) # orthogonal variance of projection of each data point to the line
		tmp = Σ²_i + V # intermediary result
		Turing.@addlogprob! -.5*(Δ_i^2 / tmp + log(abs(tmp)) + log1p(tan(θ)^2))
	end
end
...
plot_ellipses(subset_N, subset_x, subset_y, subset_Z, subset_S)
plot!(x -> mₑₓ₁₇*x + bₑₓ₁₇, label="exr 17")
plot!(x -> mₑₓ₁₇*x + bₑₓ₁₇ + move_up, linestyle=:dash, label=nothing)
plot!(x -> mₑₓ₁₇*x + bₑₓ₁₇ - move_up, linestyle=:dash, label=nothing)
plot!(x -> mₑₓ₀₁*x + bₑₓ₀₁, label="exr 01")
```
...
```{julia}
Vₑₓ₁₇bis = reduce(vcat, chainsₑₓ₁₇[:V].data)
level95 = quantile(Vₑₓ₁₇bis, [0.025, 0.975])
level99 = quantile(Vₑₓ₁₇bis, [0.005, 0.995])
density(chainsₑₓ₁₇[:V], title="V", label=nothing)
vline!(level95, label="95%")
vline!(level99, label="99%")
```
