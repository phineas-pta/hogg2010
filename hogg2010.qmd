  ...
  html:
    ...
    include-in-header:
      text: |
        ...
        .sourceCode {font-family: JuliaMono, monospace;}
        </style>
...
function plot_ellipses(ℕ, 𝕩, 𝕪, ℤ, 𝕊)
	p = plot(𝕩, 𝕪, seriestype=:scatter, label=nothing)
	for i ∈ 1:ℕ
		covellipse!(p, ℤ[i], 𝕊[i], label=nothing)
	end
	return p
end
```
...
quick look at data
```{julia}
plot(x, y, xerror=σ_x, yerror=σ_y, seriestype=:scatter, label=nothing)
```
...
```{julia}
plot(x´, y´, yerror=σ_y´, seriestype=:scatter, label=nothing)
...
@model function modelₑₓ₀₆(ℕ, 𝕩, 𝕪, σ_𝕪)
	...
	for i ∈ 1:ℕ
		𝕪̂ᵢ = b + m * 𝕩[i]
		σ_bad = sqrt(σ_𝕪[i]^2 + V_bad)

		## method 1: manually modify log likehood
		#=
		 inlier = log1p(-P_bad) + logpdf(Normal(𝕪̂ᵢ   , σ_𝕪[i]), 𝕪[i])
		outlier = log(   P_bad) + logpdf(Normal(Y_bad, σ_bad ), 𝕪[i])
		Turing.@addlogprob! logaddexp(inlier, outlier)
		=#

		## medthod 2: use convenient constructors for mixture models
		𝕪[i] ~ MixtureModel(Normal, [(𝕪̂ᵢ, σ_𝕪[i]), (Y_bad, σ_bad)], [1 - P_bad, P_bad])

	end
end

chainsₑₓ₀₆ = sample(modelₑₓ₀₆(N, x, y, σ_y), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
```
...
```{julia}
chainsₑₓ₀₇ = sample(modelₑₓ₀₆(N, x, y, σ_y ./ 2), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
summarystats(chainsₑₓ₀₇)[:, [:mean, :std, :mcse, :rhat]]
```
```{julia}
p = plot(layout=2)
...
```{julia}
chainsₑₓ₀₉_½err = sample(modelₑₓ₀₆(N´, x´, y´, σ_y´ ./ 2), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
...
chainsₑₓ₁₂ = sample(modelₑₓ₁₂(N´, x´, y´), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
summarystats(chainsₑₓ₁₂)[:, [:mean, :std, :mcse, :rhat]]
```
...
mₑₓ₁₂ = mean(chainsₑₓ₁₂[:m]);
Sₑₓ₁₂ = mean(chainsₑₓ₁₂[:S]);
plot(x´, y´, yerror=fill(sqrt(Sₑₓ₁₂), N´), seriestype=:scatter, label=nothing)
...
@model function modelₑₓ₁₃(ℕ, ℤ, 𝕊)
	...
	v⃗ = [-sin(θ) cos(θ)]' # unit vector orthogonal to the line

	for i ∈ 1:ℕ
		Δᵢ = dot(ℤ[i], v⃗) - b*v⃗[1] # orthogonal displacement of each data point from the line
		Σᵢ² = dot(v⃗', 𝕊[i], v⃗) # orthogonal variance of projection of each data point to the line
		Turing.@addlogprob! -.5*(Δᵢ^2 / Σᵢ² + log(abs(Σᵢ²)) + log1p(tan(θ)^2))
	end
end

chainsₑₓ₁₃ = sample(modelₑₓ₁₃(N´, Z´, S´), NUTS(), MCMCThreads(), N_samples, N_chains; num_warmup=N_warmup, thinning=N_thinning);
summarystats(chainsₑₓ₁₃)[:, [:mean, :std, :mcse, :rhat]]
```
...
@model function modelₑₓ₁₇(ℕ, ℤ, 𝕊)
	...
	for i ∈ 1:ℕ
		...
		Turing.@addlogprob! -.5*(Δᵢ^2 / tmp + log(abs(tmp)) + log1p(tan(θ)^2))
	end
end
...
level9x = quantile(reduce(vcat, chainsₑₓ₁₇[:V].data), [.5 + .95/2, .5 + .99/2])
